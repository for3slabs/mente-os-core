...
> Método de Fases "F" (`ESTANDAR_Metodo_Fases_F.md`). Creada 2026-07-19.
...
1. **Punteros triplicados:** la lista de ramas vive en 3 lugares sincronizados A MANO —
   ...
   `registro.md` y la prosa por rama del mismo registro. El propio código dice *"sincronizar con
...
2. **Puerta de permisos encerrada en bash:** `_permiso_lectura()` vive dentro del script `maestro`
   y parsea `permisos.md` con grep. C y D necesitan la MISMA puerta (una búsqueda que devuelva
...
1. **Punteros = `punteros.tsv`, un solo archivo.** `maestro`, `mente-os-nueva`, el indexador y el
   registro lo leen. La tabla de `registro.md` pasa a decir "generada de punteros.tsv".
...
`registro.md` referencia el tsv. *Red de seguridad:* `maestro ramas` antes/después idéntico.
...
- **D1 · Modelo + almacén del grafo.** Grafo AGE `maestro` (separado del de conceptos). Nodos:
  ...
  (contrato 4). Sembrado inicial DESDE `punteros.tsv` + `permisos.md` (ramas, dueños, carriles —
...
- Al cierre: batería §5-BIS COMPLETA + actualizar `registro.md`/`REGLAS_MAESTRO.md`/
...
*Relacionado: `Maestro/ALTERNATIVAS_QUE_CONECTAR.md` (el menú original) · `Cuerpo/Ronda_Mente_OS_Maestro.md`
(F1-F5) · `Alma/Vision_Mente_OS_Maestro_Y_Foresito_Entrenado.md` · memoria
