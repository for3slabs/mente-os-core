...
## 0-bis · TRES REGLAS HERMANAS — el resto del ciclo vive aparte

Este archivo cubre **un ticket → un PR**. Lo demás se partió al pasar del techo (297 → 250):
...
| **`rules/rule-pr-base.md`** | ⛔ **contra qué rama se abre** — encadenar sobre una sin mergear pierde el trabajo |
| **`rules/rule-post-merge-cleanup.md`** | **verificar que el trabajo viajó, luego borrar la rama** |

⛔ **Verificar SIEMPRE antes de borrar** — el squash deja trabajo fuera sin avisar (§2, anti-patrón #8): **5 veces** aquí, la última el 18-ago.
...
| 8 | A PR that depends on another un-merged PR | 🔒 **candado**: `bin/check-pr-base`. La excepción *"unless explicitly stacked"* **queda ANULADA aquí** → `rules/rule-pr-base.md` |
...
- [ ] ⛔ **`bin/check-pr-base <base>` sale 0** — la base es `master` (`rules/rule-pr-base.md`)
...
- [ ] Context file updated (commit log, known issues) · [ ] **No secrets committed**
